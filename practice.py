...
def halvesies(numbers):
    ...
    the half::
        ...
        [0.5, 2.5]
    """
    halved_nums = [float(number) / 2 for number in numbers]
...
def join_strings_with_comma(words):
    ...
    if len(words) > 1:
        for word in words[1:]:
            base_string = ", ".join([base_string, word])
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    for index in range(length / 2):
        items[index], items[length - index - 1] = items[length - index - 1], items[index]
...
def duplicates(items):
    ...
    for index in range(len(sorted_items) - 1):
        if sorted_items[index] == sorted_items[index + 1] and sorted_items[index] not in dupes:
            dupes.append(sorted_items[index])
